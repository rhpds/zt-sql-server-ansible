...
[source,bash]
----
/opt/mssql-tools18/bin/sqlcmd -S 0.0.0.0,1433 -U sa -P p@55w0rD -N -C -i ./InstPubs.sql
----
...
[source,bash]
----
/opt/mssql-tools18/bin/sqlcmd -S 0.0.0.0,1433 -U sa -P p@55w0rD -d pubs -N -C -Q "SELECT TOP 1 stores.stor_name as storename, sum(sales.qty) as sumqty FROM stores JOIN sales ON stores.stor_id = sales.stor_id GROUP BY stores.stor_id, stores.stor_name ORDER BY sumqty DESC"
----
